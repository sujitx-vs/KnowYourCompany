...
class Worker:
    ...
    def execute(self, run):
        ...
        try:
            ...
            log.exception("Research failed: run=%s phase=%s", run_id, run["phase"])
            with suppress(LeaseLost):
                finish("export_failed" if run["phase"] == "export" else "failed",
                       "The PDF could not be prepared. Your web brief is saved; retry the export." if run["phase"] == "export" else "This research step could not finish. You can retry from the last saved step.",
